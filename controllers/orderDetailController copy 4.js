...
const {product_id,quanlity,productname,orderDetail_NO,price} =req.body
  ...
  OrderDetail.findOne(con).then(async (findOrder) => {
    if (!findOrder) {
      ...
      try {
        var conn = { orderDetail_NO: findOrder.orderDetail_NO,product_id: product_id };
        console.log(conn)
